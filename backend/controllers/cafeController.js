...
const getSecondaryLatitude = (cafe) =>
  parseCoordinate(cafe?.secondary_latitude ?? cafe?.secondaryLatitude);
const getSecondaryLongitude = (cafe) =>
  parseCoordinate(cafe?.secondary_longitude ?? cafe?.secondaryLongitude);
...
const cafeController = {
  ...
  async getAllCafes(req, res) {
    try {
      ...
      res.json(
        FALLBACK_CAFES.map(({ pin: _pin, daily_pin: _daily_pin, ...publicCafe }) => publicCafe)
      );
    }
  },
  ...
  async updateLocation(req, res) {
    ...
    const hasSecondaryLatitude =
      Object.prototype.hasOwnProperty.call(body, 'secondaryLatitude') ||
      Object.prototype.hasOwnProperty.call(body, 'secondary_latitude');
    const hasSecondaryLongitude =
      Object.prototype.hasOwnProperty.call(body, 'secondaryLongitude') ||
      Object.prototype.hasOwnProperty.call(body, 'secondary_longitude');
    const hasSecondaryRadius =
      Object.prototype.hasOwnProperty.call(body, 'secondaryRadius') ||
    ...
    if (hasSecondaryUpdate) {
      ...
      if (
        (secondaryLatitudeEmpty && !secondaryLongitudeEmpty) ||
        (!secondaryLatitudeEmpty && secondaryLongitudeEmpty)
      ) {
        return res
          .status(400)
          .json({ error: 'Ek konum için enlem ve boylam birlikte girilmelidir.' });
      }
      ...
      if (clearingSecondary) {
        if (hasSecondaryRadius && !secondaryRadiusEmpty) {
          return res
            .status(400)
            .json({ error: 'Ek konumu temizlerken yarıçap değeri gönderilemez.' });
        }
        ...
        if (
          !Number.isFinite(parsedSecondaryLatitude) ||
          parsedSecondaryLatitude < -90 ||
          parsedSecondaryLatitude > 90
        ) {
          return res
            .status(400)
            .json({ error: 'Ek konum enlem değeri -90 ile 90 arasında olmalıdır.' });
        }

        const parsedSecondaryLongitude = parseDecimal(secondaryLongitudeRaw);
        if (
          !Number.isFinite(parsedSecondaryLongitude) ||
          parsedSecondaryLongitude < -180 ||
          parsedSecondaryLongitude > 180
        ) {
          return res
            .status(400)
            .json({ error: 'Ek konum boylam değeri -180 ile 180 arasında olmalıdır.' });
        }
        ...
        if (hasSecondaryRadius && !secondaryRadiusEmpty) {
          ...
          if (
            !Number.isFinite(parsedSecondaryRadius) ||
            parsedSecondaryRadius < 10 ||
            parsedSecondaryRadius > 5000
          ) {
            return res
              .status(400)
              .json({ error: 'Ek konum yarıçapı 10-5000 metre aralığında olmalıdır.' });
          }
        }
    ...
    try {
      const updates = ['latitude = $1', 'longitude = $2', 'radius = $3'];
  ...
  async checkIn(req, res) {
    ...
    if (!(await isDbConnected())) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: `Masa numarası 1-${maxTableCount} aralığında olmalıdır.` });
      }
      ...
      if (hasLocation) {
        if (locationTargets.length === 0) {
          return res.status(400).json({
            error:
              'Bu kafe için konum doğrulaması henüz ayarlanmadı. Lütfen kafe yetkilisine bildirin.',
          });
        }
        ...
        if (!accepted) {
          ...
          const nearestRadius = nearest
            ? nearest.effectiveRadius
            : getEffectiveRadius(150, parsedAccuracy);
    ...
    try {
      ...
      if (hasLocation) {
        ...
        return res.status(400).json({
          error:
            'Konum alınamadı. Giriş için konum izni verin veya geçerli masa doğrulama kodu girin.',
        });
      }

      // 3. Update User
      await pool.query('UPDATE users SET cafe_id = $1, table_number = $2 WHERE id = $3', [
        id,
        parsedTableNumber,
        userId,
      ]);
