...
describe('Socket Auth Middleware', () => {
  ...
  describe('Token Validation', () => {
    ...
    it('should reject connection when token is expired', async () => {
      const expiredToken = jwt.sign({ id: 1, username: 'testuser' }, JWT_SECRET, {
        expiresIn: '-1h',
      });
  ...
  describe('User Authentication', () => {
    const { pool, isDbConnected: _isDbConnected } = require('../db');

    it('should authenticate user with valid token', async () => {
      const validToken = jwt.sign({ id: 1, username: 'testuser' }, JWT_SECRET, { expiresIn: '1h' });
    ...
    it('should authenticate using cookie token when provided', async () => {
      const validToken = jwt.sign({ id: 2, username: 'cookieuser' }, JWT_SECRET, {
        expiresIn: '1h',
      });
    ...
    it('should prioritize cookie token over handshake token', async () => {
      const cookieToken = jwt.sign({ id: 3, username: 'cookie-priority' }, JWT_SECRET, {
        expiresIn: '1h',
      });
      const handshakeToken = jwt.sign({ id: 99, username: 'handshake-user' }, JWT_SECRET, {
        expiresIn: '1h',
      });
    ...
    it('should reject connection when user not found in database', async () => {
      const validToken = jwt.sign({ id: 999, username: 'nonexistent' }, JWT_SECRET, {
        expiresIn: '1h',
      });
      mockSocket.handshake.auth.token = validToken;
