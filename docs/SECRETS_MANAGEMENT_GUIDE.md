...
```bash
# Generate new ***REMOVED*** (64 hex characters)
openssl rand -hex 64
...
|----------|-------------|---------|
| `***REMOVED***` | JWT signing key (min 64 chars) | `openssl rand -hex 64` |
| `DB_PASSWORD` | PostgreSQL password | Random 32+ chars |
| `CORS_ORIGIN` | Allowed frontend origins | `https://example.com` |
| `***REMOVED***` | Redis connection string | `redis://localhost:6379` |
...
| `***REMOVED***` | Google OAuth secret | Google Cloud Console |
| `***REMOVED***` | reCAPTCHA secret | reCAPTCHA Admin |
