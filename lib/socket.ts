...
const enforceBrowserHttps = (
  url: string,
  protocolOverride?: Pick<Location, 'protocol'>
): string => {
  const protocol =
    protocolOverride?.protocol ?? (typeof window !== 'undefined' ? window.location.protocol : null);
...
export const normalizeBaseUrl = (
  ...
  enforceBrowserHttps(withProtocol(url.trim()), protocolOverride)
    .replace(/\/+$/, '')
    .replace(/\/api$/, '');

export const resolveSocketUrl = () => {
  const socketUrl = getViteEnvVar('VITE_SOCKET_URL');
  if (socketUrl) return normalizeBaseUrl(socketUrl);
  const apiBaseUrl = getViteEnvVar('VITE_API_BASE_URL');
  if (apiBaseUrl) return normalizeBaseUrl(apiBaseUrl);
  const apiUrl = getViteEnvVar('VITE_API_URL');
