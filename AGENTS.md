...
### Security
- **JWT_SECRET must be 64+ chars** - Generate with `openssl rand -hex 64`. Server will refuse to start otherwise.
...
- Test both paths: set invalid `DATABASE_URL` to verify memory mode works
...
- Redis-backed rate limiting requires `REDIS_URL` - falls back to in-memory if unavailable
