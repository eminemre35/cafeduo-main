...
export const Store: React.FC<StoreProps> = ({ user, updateUser, onShowToast }) => {
  ...
  useEffect(() => {
    fetchStoreData();
    // Intentionally mount-only. fetchStoreData closes over the current `user`
    // for the inventory call; subsequent user changes are reconciled by the
    // parent that re-mounts this component (route change). Adding the
    // function or `user` to deps would cause an extra fetch on every parent
    // re-render that is otherwise harmless.
  ...
  const getIconForType = (type: string) => {
    switch (type) {
      case 'rank':
        return <Award className="text-cyan-400" />;
      case 'frame':
      ...
      default:
        return <ShoppingCart className="text-[var(--rf-muted)]" />;
    }
  };
  ...
  if (loading) {
    return (
      <div className="w-full h-64 flex flex-col items-center justify-center relative noise-bg">
        ...
        <p className="mt-4 font-body text-cyan-400 tracking-widest animate-pulse">
          MARKET_AĞI_TARANIYOR...
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 pt-12 pb-32 relative noise-bg min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b-2 border-cyan-900/50 pb-6">
        <div>
          ...
          <p className="font-body text-cyan-600 tracking-widest mt-2">
            Holografik Eşya Pazarı // Lisanslı Ekipmanlar
          </p>
        </div>
        ...
        {user && (
          ...
          >
            <div className="text-right flex flex-col justify-center">
              <span className="text-[10px] font-body text-cyan-600 uppercase tracking-widest block skew-x-[10deg]">
                Aktif Bakiye
              </span>
              <span className="font-display text-3xl text-cyan-400 block skew-x-[10deg]">
                {user.points} <span className="text-sm text-pink-500">CP</span>
              </span>
            </div>
          </motion.div>
        )}
        ...
        {items.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative bg-[#050a19] border-t-2 border-l-2 border-r-4 border-b-4 p-6 flex flex-col justify-between group overflow-hidden ${
                owned
                  ? 'border-emerald-500 shadow-[6px_6px_0_rgba(16,185,129,0.2)]'
                  : afford
                    ? 'border-cyan-400 shadow-[6px_6px_0_rgba(0,243,255,0.2)] hover:border-pink-500 hover:shadow-[8px_8px_0_rgba(255,0,234,0.3)]'
            ...
            >
              ...
              <div>
                <div className="flex justify-between items-start mb-4 relative z-10">
                  <div className="w-12 h-12 bg-black border border-cyan-900/50 flex items-center justify-center skew-x-[-10deg]">
                    <div className="skew-x-[10deg]">{getIconForType(item.type)}</div>
                  </div>
                  <span
                    className={`font-display text-xl tracking-widest ${owned ? 'text-emerald-400' : afford ? 'text-pink-500' : 'text-[var(--rf-muted)]'}`}
                  >
                    {item.price} CP
                  </span>
                </div>

                <h3 className="text-2xl font-display text-white mb-2 tracking-wide relative z-10 uppercase">
                  {item.title}
                </h3>
                <p className="text-sm font-body text-cyan-800/80 mb-6 relative z-10">
                  {item.description}
                </p>
              </div>
              ...
              <div className="relative z-10 mt-auto">
                <button
                  ...
                  className={`w-full py-3 font-display text-lg tracking-widest uppercase transition-all flex items-center justify-center gap-2 skew-x-[-5deg] ${
                    owned
                      ? 'bg-emerald-950/30 text-emerald-500 border-2 border-emerald-900/50 cursor-not-allowed'
                      : afford
                        ? 'bg-cyan-950/30 text-cyan-400 border-2 border-cyan-500/50 hover:bg-pink-950/30 hover:text-pink-400 hover:border-pink-500'
