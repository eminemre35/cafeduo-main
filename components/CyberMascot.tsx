...
export const CyberMascot: React.FC<CyberMascotProps> = ({ mood, className = '' }) => {
  ...
  useEffect(() => {
    const updateSprings = () => {
      ...
      const targetRotate = nx * -10 * hoverBoost;
  ...
  const getColors = () => {
    switch (activeMood) {
      case 'happy':
        return { eye: '#4ade80', glow: 'rgba(74, 222, 128, 0.4)' };
      case 'angry':
        return { eye: '#f43f5e', glow: 'rgba(244, 63, 94, 0.5)' };
      case 'typing':
        return { eye: '#eab308', glow: 'rgba(234, 179, 8, 0.4)' };
      default:
        return { eye: '#00f3ff', glow: 'rgba(0, 243, 255, 0.4)' };
    }
  };
    ...
    <div
      ...
      onPointerMove={(e) => {
        ...
        if (local) {
          ...
          const targetRotate = local.x * -10 * hoverBoost;
          if (activeMood === 'angry') {
            targetEyeX += (Math.random() - 0.5) * 2;
            targetEyeY += (Math.random() - 0.5) * 2;
          } else if (activeMood === 'happy') {
    ...
    >
      ...
      >
        <motion.div
          ...
          animate={
            isHovered ? { opacity: [0.25, 0.75, 0.25], scale: [0.98, 1.06, 0.98] } : { opacity: 0 }
          }
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  ...
                  clipPath:
                    activeMood === 'angry' ? 'polygon(0 30%, 100% 0, 100% 100%, 0 100%)' : 'none',
                }}
              />
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: `0 0 15px ${colors.glow}`,
                  clipPath:
                    activeMood === 'angry' ? 'polygon(0 0, 100% 30%, 100% 100%, 0 100%)' : 'none',
                }}
              />
            </motion.div>
          </div>
